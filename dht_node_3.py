...
def Find_Shortcut():
    global shortcutnode, SHORTCUT_AVA, last_sct_reply, self_identifier
    SHORTCUT_AWAITING_PORT = 23333 + self_identifier
    timeout = SHORTCUT_NUMBER * NODE_TIMEOUT_INTERVAL
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sock.bind((socket.gethostname(), SHORTCUT_AWAITING_PORT))
    # sock.settimeout(timeout)
    # TODO how to solve the unblocking socket reading with time out
    sock.listen(1)
    if SUCNODE1_AVA:
        try:
            # Send out the searching request SCT:[shortcutcount]:[src_ip]:[src_waiting port]
            Send_TCP_msg("SCT:" + bytes(SHORTCUT_NUMBER) + ":" + bytes(socket.gethostname()) + ":" + bytes(
                SHORTCUT_AWAITING_PORT), sucnode_1[0], sucnode_1[1])
            conn, addr = sock.accept()
            str = conn.recv(BUFFER)
            if str == "SCTISME":
                shortcutnode = (addr[0], int(addr[1]))
                conn.close()
                sock.close()
                SHORTCUT_AVA = True
                printbycom("Shortcut found !" + bytes(shortcutnode), SHOW_TRIVAL_MSG)
                # Set the last received shortcut time to current time to prevent timeout in the first round
                last_sct_reply = time.time()
        except socket.timeout as e:
            SHORTCUT_AVA = False
            print("Shortcut is not found")
            print(e.message)
    else:
        printbycom("Sucnode is not available for sending out request", SHOW_TRIVAL_MSG)
...
def Status_monitor():
    ...
    while True:
        ...
        if SUCNODE1_AVA and (
                time.time() - last_suc_reply > NODE_TIMEOUT_INTERVAL):  # if the sucnode is timeout ,try to contact sucnode_2
        ...
        if SHORTCUT_AVA and (
                time.time() - last_sct_reply > NODE_TIMEOUT_INTERVAL):  # if the sucnode is timeout try to contact sucnode_2
            SHORTCUT_AVA = False
            threading.Thread(target=Find_Shortcut).start()
    ...
    while True:
        ...
        while True:
            ...
            if command == "STORE" or command == "REQ":
                ...
                printbycom("Node" + bytes(addr[0]) + ":" + bytes(addr[1]) + "is" + command.lower() + "for" + filename,
                           SHOW_TRIVAL_MSG)
            ...
            if command == "SCT":
                ...
                if searchcount == 1:
                    ...
                    Send_TCP_msg("SCTISME", src_ip, src_port)
                else:
                    # Else we just decrease the search count by 1 and forward the request
                    searchcount = searchcount - 1
                    if SUCNODE1_AVA:
                        Send_TCP_msg("SCT:" + bytes(searchcount) + ":" + src_ip + ":" + src_port, sucnode_1[0],
                                     suc_id + TCP_PORT_BASE)
                break
